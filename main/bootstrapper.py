...
# класс, который обновляет и устанавливает pip
class Pip:
...
class Main:
    def __init__(self, master):
        ...
        # запуск проверки модулей (check_pip)
        try:
            self.check_pip()
        except Exception:
            pass
    ...
    def check_pip(self):
        """
        cwd по желанию,
        /k - не закрывает окно консоли после выхода,
        в main_path написать имя вашего файла который будет запускаться
        """

        try:
            import colorama

            main_path = "example.py"
            subprocess.Popen(
                ["cmd", "/k", sys.executable, str(main_path)],
                cwd="",
